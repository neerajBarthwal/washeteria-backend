...
public class AuthorizationFilter extends OncePerRequestFilter {
	...
	protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
		...
		if(authorizationHeader!=null && authorizationHeader.startsWith("Bearer ")) {
			try {
				...
				invalidateRequest(response, new ErrorMessage(ExceptionCodes.TAMPERED_TOKEN));
			}catch(ExpiredJwtException expiredException) {
				invalidateRequest(response, new ErrorMessage(ExceptionCodes.EXPIRED_TOKEN));
			}catch(MalformedJwtException malformedJwtException) {
				invalidateRequest(response, new ErrorMessage(ExceptionCodes.MALFORMED_TOKEN));
			}
			...
			//Authorize the user with IIIT-H reverse proxy server and create a JWT.
				...
				String message = serializeToJson(new ErrorMessage(ExceptionCodes.INVALID_CREDENTIALS));
    ...
    protected boolean shouldNotFilter(HttpServletRequest request) throws ServletException {
        ...
        if(path.startsWith("/events")&& request.getMethod().equals("GET"))
        	return true;
        else if(path.startsWith("/events") || path.startsWith("/auth/token"))
        	return false;
        return true;
    }
